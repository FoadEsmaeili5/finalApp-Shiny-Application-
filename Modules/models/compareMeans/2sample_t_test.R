...
fluidRow(
  column(width = 2,offset = 0,
         ...
         checkboxInput(inputId = ns("covequal"),label = "equality variance assumed",value = T),
...
column(width = 4
       # bsCollapsePanel("introducing 2 sample t test"
       # )
)
...
server_2sample_t_test=function(input,output,session,data){
  ns=session$ns
  ...
  model=eventReactive(input$submit,{
    if(!input$formulaT){
      t.test(data()%>%
             ...
             var.equal=input$covequal,alternative = input$typeTest)
    }else{
      t.test(newData()%>%
               DF2formula(),data=newData(),na.rm=T,var.equal=input$covequal,alternative = input$typeTest)
      }
  })
...
ui=dashboardPage(header = dashboardHeader(),sidebar = dashboardSidebar(),body = dashboardBody(
  ui_2sample_t_test("foad")
))
server=function(input,output){
  callModule(server_2sample_t_test,"foad",reactive(iris))
}
